...
const getPowNumber = (number, power) => {
    ...
    if (power > 0 && indicate) {
        ...
        for (let i = 1; i <= Math.abs(power); i++) powNumber = powNumber / number;
        return powNumber;
    } else {
        powNumber;
    }
...
const getRealSalary = (salary) => {
    console.log(`Номінальна зарплата: ${salary}`);
    for (let char of salary) {
        if (isNaN(+char)) salary = salary.slice(0, salary.indexOf(char)) + salary.slice(salary.indexOf(char) + 1);
    }
    console.log(`Зарплата в нормальному вигляді: ${salary}`);
